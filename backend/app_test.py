...
def generate_test_data():
    ...
    for i, timestamp in enumerate(time_range):
        ...
        value = max(30.0, min(95.0, base_usage + trend + seasonal + noise))
...
def load_metric_data():
    ...
    try:
        # 尝试使用真实数据文件
        data_file = '/app/data/tb_metric_raw_free_space_ratio_202509161435.csv'
        if os.path.exists(data_file):
            df = pd.read_csv(data_file)
        else:
            # 如果真实数据不存在，生成测试数据
            print("真实数据文件不存在，使用生成的测试数据")
            df = generate_test_data()
    except Exception as e:
        print(f"加载数据出错，使用生成的测试数据: {e}")
        df = generate_test_data()
...
def get_data_info():
    ...
    try:
        ...
        response = {
            ...
            'sample_data': {
                'dates': series_data.index[-100:].strftime('%Y-%m-%d %H:%M:%S').tolist(),
                'values': series_data.values[-100:].tolist()
            }
        }
